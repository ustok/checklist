...
public class ProtocolEditor extends AbstractFileEditor {
	...
	public void createPartControl(Composite pParent) {
		...
		ProtocolControl control = new ProtocolControl(sc, SWT.NONE);
		sc.setContent(control);
		sc.setMinSize(control.computeSize(SWT.DEFAULT, SWT.DEFAULT));
		sc.layout();

		fControl = sc;
		fControlProtocol = control;

		updateUI();
	}

	private void updateUI() {
		fControlProtocol.setModel(getModel());
	}
		...
		if (file != null) {
			try {
				if (file.getContents().available() <= 0) {
					...
					newModel = factory.create();
				} else {
					newModel = (Protocol) EmfFileIoFactory.INSTANCE.createReader().read(file);
				}
			} catch (Exception ex) {
